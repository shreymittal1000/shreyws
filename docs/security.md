...
The owner-agent pilot was treated as a class-B owner-trusted workload and is now decommissioned from the live runtime. It remains as a reference for no Docker socket, no privileged mode, no broad host mounts, no command execution, owner-only Authentik group authorization, dedicated state/secrets paths, and a dedicated frontend network.
