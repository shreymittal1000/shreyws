...
async function loadAssistant(){try{const s=await api('/assistant/status');aiStatus.textContent=s.authenticated?s.version+' · signed in':s.version+' · sign-in required'}catch(e){aiStatus.textContent='assistant unavailable'}}chatForm.onsubmit=async e=>{e.preventDefault();const message=chatInput.value.trim();if(!message)return;chatOutput.textContent='Codex is thinking…';try{const d=await api('/assistant/chat',{method:'POST',body:JSON.stringify({message})});chatOutput.textContent=d.reply}catch(e){chatOutput.textContent=e.message}};if(matchMedia('(pointer:fine)').matches){addEventListener('pointermove',e=>{terminalCursor.style.left=e.clientX+'px';terminalCursor.style.top=e.clientY+'px';document.body.classList.add('cursor-active')},{passive:true});addEventListener('pointerleave',()=>document.body.classList.remove('cursor-active'));addEventListener('pointerdown',()=>terminalCursor.style.transform='translate(-50%,-50%) scale(.72)');addEventListener('pointerup',()=>terminalCursor.style.transform='translate(-50%,-50%)')}loadAll();loadAssistant();setInterval(loadAll,15000);setInterval(loadAssistant,60000);
</script></body></html>'''
