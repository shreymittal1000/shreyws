...
def system_summary() -> dict[str, object]:
    ...
    return {
        ...
        "load": load, "uptime_seconds": int(float((HOST_PROC / "uptime").read_text().split()[0])),
...
:root{--bg:#080b12;--panel:#101622;--line:#253047;--text:#e9efff;--muted:#8e9bb5;--accent:#7c9cff;--green:#45d39a;--red:#ff6f85;--amber:#ffca68}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 10% 0,#18213c 0,transparent 35%),var(--bg);color:var(--text);font:14px/1.45 Inter,ui-sans-serif,system-ui,sans-serif}header{padding:28px clamp(18px,5vw,72px) 18px;display:flex;justify-content:space-between;align-items:end}h1{margin:0;font-size:clamp(28px,4vw,48px);letter-spacing:-.04em}.eyebrow{color:var(--accent);text-transform:uppercase;letter-spacing:.18em;font-size:11px;font-weight:800}.muted{color:var(--muted)}main{padding:0 clamp(18px,5vw,72px) 70px;max-width:1600px;margin:auto}.grid{display:grid;grid-template-columns:repeat(12,1fr);gap:16px}.card{background:linear-gradient(145deg,rgba(20,28,44,.96),rgba(12,17,27,.96));border:1px solid var(--line);border-radius:18px;padding:20px;box-shadow:0 18px 60px #0005}.metric{grid-column:span 3}.metric b{display:block;font-size:26px;margin:8px 0}.apps{grid-column:span 8}.deploy{grid-column:span 4}.events{grid-column:span 12}h2{margin:0 0 16px;font-size:18px}button,.button{border:0;border-radius:9px;padding:9px 12px;background:#25314b;color:var(--text);cursor:pointer;font-weight:700}button.primary{background:var(--accent);color:#071027}button.danger{background:#5b2330;color:#ffdce2}button:hover{filter:brightness(1.15)}input,select,textarea{width:100%;background:#090e18;border:1px solid var(--line);border-radius:9px;color:var(--text);padding:10px;margin:5px 0 12px}label{color:var(--muted);font-size:12px;font-weight:700}.row{display:grid;grid-template-columns:1fr 1fr;gap:12px}.app{border-top:1px solid var(--line);padding:16px 0}.app:first-of-type{border-top:0}.app-head,.actions{display:flex;gap:8px;align-items:center;justify-content:space-between;flex-wrap:wrap}.pill{padding:3px 8px;border-radius:99px;background:#263147;color:var(--muted);font-size:11px}.pill.running{background:#123d31;color:var(--green)}pre{white-space:pre-wrap;max-height:280px;overflow:auto;background:#070a10;padding:12px;border-radius:10px;color:#becaf0}.bar{height:7px;background:#222b3e;border-radius:8px;overflow:hidden}.bar i{display:block;height:100%;background:linear-gradient(90deg,var(--accent),#9c6fff)}dialog{background:var(--panel);color:var(--text);border:1px solid var(--line);border-radius:16px;width:min(850px,92vw)}dialog::backdrop{background:#000b}@media(max-width:950px){.metric{grid-column:span 6}.apps,.deploy{grid-column:span 12}}@media(max-width:520px){.metric{grid-column:span 12}.row{grid-template-columns:1fr}}
</style></head><body><header><div><div class="eyebrow">Personal cloud control plane</div><h1>ShreyWS Launchpad</h1><div class="muted">Deploy deliberately. Observe everything.</div></div><button onclick="loadAll()">Refresh</button></header><main><div id="metrics" class="grid"></div><div class="grid" style="margin-top:16px"><section class="card apps"><h2>Applications</h2><div id="apps">Loading…</div></section><section class="card deploy"><h2>Launch application</h2><form id="form"><label>App name</label><input name="name" placeholder="my-app" required pattern="[a-z][a-z0-9-]{1,38}[a-z0-9]"><label>Source type</label><select name="source_type" id="sourceType"><option value="image">Approved image</option><option value="git">Git repository with Dockerfile</option></select><label>Image or HTTPS repository</label><select name="image" id="image"></select><input name="git" id="git" placeholder="https://github.com/user/repo.git" hidden><div class="row"><div><label>RAM (MiB)</label><input type="number" name="memory_mb" value="256" min="32" max="16384"></div><div><label>CPU cores</label><input type="number" name="cpus" value="0.5" min="0.1" max="4" step="0.1"></div></div><div class="row"><div><label>Persistent storage (GiB)</label><input type="number" name="storage_gb" value="1" min="0" max="500"></div><div><label>Container port</label><input type="number" name="container_port" value="80" min="1" max="65535"></div></div><label>Visibility</label><select name="visibility"><option value="internal">Internal · Tailscale</option><option value="public">Public · requires ingress</option></select><label>Domain/subdomain (optional)</label><input name="domain" placeholder="app.example.com"><label>Fixed internal IP (advanced, optional)</label><input name="ipv4" placeholder="172.30.x.x"><label>Environment · one KEY=value per line</label><textarea name="environment" rows="3"></textarea><label>Secrets · one KEY=value per line</label><textarea name="secrets" rows="3"></textarea><button class="primary" type="submit">Deploy application</button></form><p id="notice" class="muted"></p></section><section class="card events"><h2>Recent activity</h2><div id="events"></div></section></div></main><dialog id="logs"><div class="app-head"><h2 id="logTitle">Logs</h2><button onclick="logs.close()">Close</button></div><pre id="logText"></pre></dialog><script>
