...
<title>ShreyWS Launchpad</title><link rel="stylesheet" href="''' + BASE + r'''/assets/xterm.css"><style>
...
.loading-value{color:var(--accent)!important;letter-spacing:.18em;animation:cursor-blink 1.05s steps(1,end) infinite}.loading-bar{width:14%!important;animation:cursor-blink 1.05s steps(1,end) infinite}.shell-command{display:flex;align-items:center;gap:8px;margin-top:12px;padding:8px 10px;background:#010302;border:1px solid #102a18}.shell-command code{flex:1;overflow-x:auto;white-space:nowrap;color:#9ceead;font-size:12px}.shell-command button{padding:6px 9px;flex:none}.terminal-screen{height:min(68vh,680px);max-height:none;overflow:hidden;margin:14px 0;background:#000;padding:8px;border:1px solid #102a18;white-space:normal}.terminal-screen .xterm{height:100%}.terminal-screen .xterm-viewport{overflow-y:auto!important}.terminal-form{display:grid;grid-template-columns:1fr auto auto;gap:8px;align-items:start}.terminal-form input{margin:0}.terminal-status{color:var(--muted)}@media(prefers-reduced-motion:reduce){.loading-value,.loading-bar{animation:none}}
...
async function copyShellCommand(button,command){try{await navigator.clipboard.writeText(command);const previous=button.textContent;button.textContent='Copied';setTimeout(()=>button.textContent=previous,1400)}catch{prompt('Copy this shell command:',command)}}
async function loadAll(){try{const d=await api('/overview');const m=d.system.memory,st=d.system.storage,c=d.system.cpu_count,l=+d.system.load[0];metrics.innerHTML=[['Host memory',fmt(m.used)+' / '+fmt(m.total),m.used/m.total,'Available memory included'],['Assigned RAM',d.system.assigned_memory_mb+' MiB',d.system.assigned_memory_mb/(m.total/1048576),'Limits assigned to Launchpad apps'],['Storage',fmt(st.used)+' / '+fmt(st.total),st.used/st.total,'Used on main application storage'],['System load · 1m / 5m / 15m',d.system.load.join(' · '),Math.min(1,l/c),`${loadState(l,c)} · ${c} CPU cores available`]].map(x=>`<section class="card metric"><span class="muted">${x[0]}</span><b>${x[1]}</b><div class="muted" style="min-height:21px">${x[3]}</div><div class="bar"><i style="width:${Math.round(x[2]*100)}%"></i></div></section>`).join('');const managed=d.apps.length?d.apps.map(a=>{const shellCommand=a.source===HERMES_IMAGE?`docker exec -it -u hermes shreyws-app-${a.name} /bin/bash`:`docker exec -it shreyws-app-${a.name} /bin/sh`;return `<div class="app"><div class="app-head"><div><b>${esc(a.name)}</b> <span class="pill ${esc(a.state)}">${esc(a.state)}</span><div class="muted">${esc(a.source)} · ${a.memory_mb} MiB · ${a.cpus} CPU · ${esc(a.stats.memory||'—')}</div><a href="${esc(a.url)}" target="_blank" class="muted">${esc(a.url)}</a></div><div class="actions"><button onclick="act('${a.name}','start')">Start</button><button onclick="act('${a.name}','stop')">Stop</button><button onclick="act('${a.name}','restart')">Restart</button><button onclick="act('${a.name}','update')">Update</button><button onclick="openTerminal('${a.name}')" ${a.state==='running'?'':'disabled'}>Terminal</button><button onclick="showLogs('${a.name}')">Logs</button><button class="danger" onclick="removeApp('${a.name}')">Remove</button></div></div><div class="shell-command"><code>${esc(shellCommand)}</code><button type="button" onclick="copyShellCommand(this,'${shellCommand}')">Copy shell command</button></div></div>`}).join(''):'<p class="muted">No Launchpad-managed applications yet.</p>';apps.innerHTML=managed+externalHtml(d.external_apps||[]);events.innerHTML=d.events.map(e=>`<span class="pill">${new Date(e.created_at*1000).toLocaleString()} · ${esc(e.actor)} · ${esc(e.app||'platform')} · ${esc(e.action)}: ${esc(e.outcome)}</span>`).join(' ');image.innerHTML=d.approved_images.map(i=>`<option value="${esc(i.image)}" data-port="${i.port}">${esc(i.label)} · ${esc(i.image)}</option>`).join('');image.onchange=()=>{if(sourceType.value==='image')form.container_port.value=image.selectedOptions[0].dataset.port};image.onchange()}catch(e){notice.textContent=e.message}}
const HERMES_IMAGE='nousresearch/hermes-agent:v2026.8.27';let configuredImage='';const baseLoadAll=loadAll;loadAll=async function(){const selectedImage=image.value,selectedPort=form.container_port.value;await baseLoadAll();if(selectedImage&&[...image.options].some(option=>option.value===selectedImage)){image.value=selectedImage;form.container_port.value=selectedPort}configureImageDefaults();for(const card of apps.querySelectorAll('.app')){if(!card.textContent.includes(HERMES_IMAGE))continue;const actions=card.querySelector('.actions'),name=card.querySelector('b')?.textContent;if(!actions||!name||actions.querySelector('.hermes-setup'))continue;const button=document.createElement('button');button.className='primary hermes-setup';button.textContent='Set up Hermes';button.onclick=()=>setupHermes(name);const terminal=[...actions.querySelectorAll('button')].find(item=>item.textContent==='Terminal');actions.insertBefore(button,terminal||null)}};function configureImageDefaults(){if(sourceType.value!=='image')return;const selected=image.value;if(selected===configuredImage)return;configuredImage=selected;if(selected!==HERMES_IMAGE)return;form.memory_mb.value=4096;form.cpus.value=2;form.storage_gb.value=25;form.container_port.value=8642;portHint.textContent='Persistent Hermes workspace. These are suggested defaults; you can change them.'}image.addEventListener('change',configureImageDefaults);function setupHermes(name){openTerminal(name);const deadline=Date.now()+10000,timer=setInterval(()=>{if(terminalSocket?.readyState===WebSocket.OPEN){clearInterval(timer);const command='hermes setup && hermes gateway start';appendTerminal('$ '+command+'\n');terminalSocket.send(command+'\r')}else if(Date.now()>deadline)clearInterval(timer)},100)}
...
let terminalSocket=null,terminalEmulator=null,terminalFit=null,terminalObserver=null;function appendTerminal(value){terminalEmulator?.write(value)}async function terminalClasses(){if(window.XtermTerminal)return;if(!window.XtermReady)window.XtermReady=new Promise(resolve=>window.addEventListener('xterm-ready',resolve,{once:true}));await window.XtermReady}async function openTerminal(name){closeTerminal(false);await terminalClasses();terminalEmulator?.dispose();terminalObserver?.disconnect();terminalOutput.textContent='';terminalOutput.className='terminal-screen';terminalForm.hidden=true;terminalEmulator=new window.XtermTerminal({cursorBlink:true,cursorStyle:'block',fontFamily:'IBM Plex Mono, JetBrains Mono, SFMono-Regular, Consolas, monospace',fontSize:14,scrollback:5000,theme:{background:'#000000',foreground:'#d8ffe1',cursor:'#42ff72',selectionBackground:'#245c32'}});terminalFit=new window.XtermFitAddon();terminalEmulator.loadAddon(terminalFit);terminalEmulator.open(terminalOutput);terminalObserver=new ResizeObserver(()=>{try{terminalFit.fit()}catch{}});terminalObserver.observe(terminalOutput);terminalFit.fit();terminalTitle.textContent=name+' · terminal';terminalStatus.textContent='Authenticating owner session…';terminalDialog.showModal();terminalFit.fit();const scheme=location.protocol==='https:'?'wss':'ws';terminalSocket=new WebSocket(`${scheme}://${location.host}${B}/apps/${name}/terminal`);terminalEmulator.onData(data=>{if(terminalSocket?.readyState===WebSocket.OPEN)terminalSocket.send(data)});terminalEmulator.onResize(({cols,rows})=>{if(terminalSocket?.readyState===WebSocket.OPEN)terminalSocket.send(`\0resize:${cols}:${rows}`)});terminalSocket.onopen=()=>{terminalStatus.textContent='Connected · interactive TTY · 30 minute limit';terminalSocket.send(`\0resize:${terminalEmulator.cols}:${terminalEmulator.rows}`);terminalEmulator.focus()};terminalSocket.onmessage=event=>terminalEmulator.write(event.data);terminalSocket.onerror=()=>{terminalStatus.textContent='Connection failed';terminalEmulator.write('\r\nTerminal connection failed. Refresh your Authentik session and try again.\r\n')};terminalSocket.onclose=event=>{terminalStatus.textContent=`Disconnected${event.code&&event.code!==1000?' · code '+event.code:''}`;terminalSocket=null}}function closeTerminal(closeDialog=true){if(terminalSocket){terminalSocket.close(1000,'owner closed terminal');terminalSocket=null}terminalObserver?.disconnect();if(closeDialog&&terminalDialog.open)terminalDialog.close()}function interruptTerminal(){if(terminalSocket?.readyState===WebSocket.OPEN)terminalSocket.send('\x03')}terminalForm.onsubmit=event=>event.preventDefault();terminalDialog.addEventListener('cancel',event=>{event.preventDefault();closeTerminal()});
...
</script><script type="module">import{Terminal}from'./assets/xterm.mjs';import{FitAddon}from'./assets/addon-fit.mjs';window.XtermTerminal=Terminal;window.XtermFitAddon=FitAddon;window.dispatchEvent(new Event('xterm-ready'));</script></body></html>'''
...
class Handler(BaseHTTPRequestHandler):
    ...
    def send_asset(self, name: str, content_type: str) -> None:
        if name not in {"xterm.css", "xterm.mjs", "addon-fit.mjs"}:
            raise LaunchpadError("Unknown static asset")
    ...
    def handle_terminal(self, name: str) -> None:
        ...
        try:
            ...
            process = subprocess.Popen(
                ["docker", "exec", "-it", "-e", "TERM=xterm-256color", container, "/bin/sh"],
            ...
            while not stop.is_set() and time.monotonic() < deadline:
                ...
                if opcode not in {1, 2}:
                    continue
                if payload.startswith(b"\0resize:"):
                    try:
                        cols, rows = (int(value) for value in payload[8:].decode("ascii").split(":", 1))
                        if not 20 <= cols <= 400 or not 5 <= rows <= 200:
                            raise ValueError
                        fcntl.ioctl(master_fd, termios.TIOCSWINSZ, struct.pack("HHHH", rows, cols, 0, 0))
                    except (UnicodeDecodeError, ValueError):
                        raise LaunchpadError("Invalid terminal dimensions")
                    continue
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path == "/assets/xterm.css": self.send_asset("xterm.css", "text/css; charset=utf-8"); return
            if path == "/assets/xterm.mjs": self.send_asset("xterm.mjs", "text/javascript; charset=utf-8"); return
            if path == "/assets/addon-fit.mjs": self.send_asset("addon-fit.mjs", "text/javascript; charset=utf-8"); return
